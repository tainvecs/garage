...
class MeilisearchAPIServer():
    ...
    def __init__(self, name, host="0.0.0.0", port=5000, debug=False):
        ...
        # init API
        self.add_endpoint(endpoint="/ms/v1/docs", endpoint_name="search", handler=self.search, req_methods=["GET"])
        self.add_endpoint(endpoint="/ms/v1/docs", endpoint_name="index", handler=self.index, req_methods=["POST"])
